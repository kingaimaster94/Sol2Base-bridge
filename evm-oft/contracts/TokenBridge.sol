...
contract TokenBridge is ITokenBridge, ReentrancyGuard, OFT {
        ...
        returns (uint nativeFee, uint zroFee)
    {
        ...
        return
            lzEndpoint.estimateFees(solanaChainId, address(this), payload, payInZRO, _adapterParams);
    }
    ...
    function setSolanaChainId(uint16 _solanaChainId) external onlyOwner {
        solanaChainId = _solanaChainId;
        emit SetsolanaChainId(_solanaChainId);
    }
    ...
    function _nonblockingLzReceive(
        ...
        require(_srcChainId == solanaChainId, "TokenBridge: invalid source chain id");
    ...
    function _checkAdapterParams(bytes calldata _adapterParams) internal view {
        if (useCustomAdapterParams) {
            _checkGasLimit(solanaChainId, uint16(PacketType.SEND_TO_APTOS), _adapterParams, 0);
        } else {
